...
def _resolve_registry_read_path(target: Path) -> Path:
    """DEV MODE read-your-writes: prefer the sandboxed registry copy.

    When dev mode is active and a sandboxed registry copy exists
    (seeded by a previous dev-mode write, or generated by
    ``ck sandbox setup``), reads come from THAT copy so chained dev
    commands compose and the fixture environment is visible to
    dashboards — mirroring the PLAN.md read-your-writes semantics in
...
class Registry:
    ...
    def save(self, path: Path | None = None) -> None:
        ...
        DEV MODE: the target (and its parent-directory creation) is
        resolved through the sandbox interceptor FIRST, so saving can
        never create or mutate the real ``~/.config/ck/`` directory —
        previously the raw ``mkdir`` on the real parent leaked an
        empty host config dir even when the write itself was
        redirected.
        """
    ...
    def load(cls, path: Path | None = None, *, locked: bool = True) -> "Registry":
        ...
        DEV MODE read-your-writes: when a sandboxed copy of the
        registry exists (``.sandbox/config/projects.json``, seeded
        by a prior dev-mode write or by ``ck sandbox setup``), the
        copy is read instead of the real file; otherwise reads come
        from the real registry.
        """
        target = path or GLOBAL_REGISTRY_FILE

        def _do_load() -> "Registry":
            read_path = _resolve_registry_read_path(target)
            migrated = cls._migrate_legacy_if_needed(read_path)
    ...
    def _migrate_legacy_if_needed(cls, target: Path) -> "Registry | None":
        ...
        DEV MODE: migration is skipped entirely — a dev session must
        never unlink the real legacy config (the sandboxed registry
        simply starts empty instead).
        """
        if target.exists():
            return None
        try:
